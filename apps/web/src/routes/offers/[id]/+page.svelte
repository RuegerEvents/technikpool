<script lang="ts">
	...
	import OfferDetail from './offer-detail.svelte';

	const offerId = $derived(page.params.id as string);
</script>

<!--
  The offer is one `await` away, and an `await` in a component holds back
  ...
  opening a second offer would sit on the first one until the new one arrived.
