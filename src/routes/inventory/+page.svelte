<script lang="ts">
	...
	const columns: Column<Row>[] = [
		...
		{ key: 'manufacturer', label: 'Manufacturer', sortable: true, accessor: (r: Row) => r.manufacturer.name },
		...
		{ key: 'maintenance', label: 'Maintenance', sortable: true, accessor: (r: Row) => r.maintenance },
...
<div class="space-y-6">
	<div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
		<div>
			...
			<p class="text-muted-foreground">Product catalog — equipment models and their stock levels.</p>
		</div>
		<select
	...
	{#key filterOrgId}
		...
		>
			{#snippet card(row)}
				<div class="rounded-lg border bg-card p-4 space-y-3 transition-colors group-hover:bg-muted/50">
					<div>
						<p class="font-semibold">{row.name}</p>
						<p class="text-sm text-muted-foreground">{row.manufacturer.name}</p>
					</div>
					<div class="grid grid-cols-2 gap-2 text-sm">
						<div class="rounded bg-green-100 dark:bg-green-900/30 px-2 py-1 text-center">
							<div class="text-lg font-bold text-green-700 dark:text-green-400">{row.available}</div>
			...
			{#snippet cell(row, key)}
				...
				{:else if key === 'available'}
					<span class="font-mono tabular-nums text-green-700 dark:text-green-400">{row.available}</span>
				{:else if key === 'maintenance'}
					<span class="font-mono tabular-nums {row.maintenance > 0 ? 'text-yellow-600 dark:text-yellow-400' : 'text-muted-foreground'}">{row.maintenance}</span>
				{:else if key === 'broken'}
					<span class="font-mono tabular-nums {row.broken > 0 ? 'text-red-600 dark:text-red-400' : 'text-muted-foreground'}">{row.broken}</span>
				{/if}
			{/snippet}
		</DataView>
	{/key}
