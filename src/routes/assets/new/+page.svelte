<script lang="ts">
	...
	let items = $state<{ serialNumber: string; assetTag: string }[]>([{ serialNumber: '', assetTag: '' }]);
	...
	$effect(() => { if (browser) localStorage.setItem('asset_create_more', String(createMore)); });
	...
	async function handleSubmit(e: Event) {
		...
		try {
			...
			if (createMore) { resetForm(); saving = false; }
			else goto(count === 1 ? `/inventory/${created[0].id}` : '/assets');
		} catch (err: any) {
			...
			{#if true}
				...
				<form onsubmit={handleSubmit} class="space-y-6">
					<div class="space-y-2">
						...
						<select id="org" bind:value={selectedOrgId} required
					...
					{#if true}
						...
						<div class="space-y-2">
							...
							<CreatableSelect items={manufacturers} value={manufacturer} onchange={handleManufacturerChange} placeholder="Search or create manufacturer…" />
						</div>
					{/if}
					...
					{#if manufacturer}
						{#key manufacturerKey}
							...
							<div class="space-y-2">
								...
								<CreatableSelect items={products} value={product} onchange={(sel) => (product = sel)} placeholder="Search or create product…" />
							</div>
						{/key}
					{/if}
					...
					<div class="space-y-2">
						...
						<Input id="quantity" type="number" min="1" max="50" value={quantity}
							oninput={(e) => setQuantity(Number((e.target as HTMLInputElement).value))} class="w-24" />
					</div>
								...
								<tbody>
									{#each items as item, i}
										<tr class="border-b last:border-0">
											...
											<td class="px-3 py-2"><Input bind:value={item.serialNumber} placeholder="S/N 123456" class="h-8 text-sm" /></td>
											<td class="px-3 py-2"><Input bind:value={item.assetTag} placeholder="TAG-001" class="h-8 text-sm" /></td>
										</tr>
									{/each}
								</tbody>
							</table>
					...
					<div class="pt-2 flex flex-col gap-4">
						...
						<div class="flex justify-end gap-4">
							<Button type="button" variant="outline" href="/assets">Cancel</Button>
