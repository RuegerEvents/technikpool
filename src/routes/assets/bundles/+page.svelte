<script lang="ts">
	...
	async function handleCreate(e: Event) {
		...
		saving = true;
		try {
			await createBundle({ name: name.trim(), description: description.trim() || undefined, organizationId: selectedOrgId });
				...
				<Card.Content class="pt-6">
					<form onsubmit={handleCreate} class="space-y-4">
						...
						<div class="flex justify-end gap-3">
							...
							<Button type="submit" disabled={saving}>{saving ? 'Creating…' : 'Create Bundle'}</Button>
						</div>
					</form>
				</Card.Content>
			</Card.Root>
		...
		{#if bundles.length === 0}
			<Card.Root>
				<Card.Content class="flex flex-col items-center justify-center py-12 text-center">
					...
					<p class="text-sm text-muted-foreground">Create bundles to group equipment for easy booking.</p>
					<Button class="mt-4" variant="outline" onclick={() => (showForm = true)}>Create your first bundle</Button>
				</Card.Content>
			</Card.Root>
		{:else}
			<div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
				{#each bundles as bundle}
					<a href="/assets/bundles/{bundle.id}" class="group block">
						<Card.Root class="h-full transition-colors hover:bg-muted/50">
							...
							<Card.Content>
								...
								{/if}
								<p class="text-sm"><span class="font-medium">{bundle.assets.length}</span> <span class="text-muted-foreground">item{bundle.assets.length !== 1 ? 's' : ''}</span></p>
							</Card.Content>
						</Card.Root>
					</a>
				{/each}
